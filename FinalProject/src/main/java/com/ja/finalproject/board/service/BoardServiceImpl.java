...
public class BoardServiceImpl {
	...
	// 좋아요	
	public void toggleLikeArticle(ArticleLikeDto articleLikeDto) {
		
		if(boardSqlMapper.countMyLike(articleLikeDto) > 0) {
			boardSqlMapper.deleteLike(articleLikeDto);
		}else {
			boardSqlMapper.insertLike(articleLikeDto);
		}
	}
	
	public int getTotalLikeCount(int articleId) {
		return boardSqlMapper.countLikeTotal(articleId);
	}
	
	public boolean isLikeArticle(ArticleLikeDto articleLikeDto) {
		return boardSqlMapper.countMyLike(articleLikeDto) > 0 ? true : false;
	}
